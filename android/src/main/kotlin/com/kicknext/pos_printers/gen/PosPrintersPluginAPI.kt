...
data class XPrinterDTO (
 ...
 {
  companion object {
    fun fromList(pigeonVar_list: List<Any?>): XPrinterDTO {
...
interface POSPrintersApi {
  ...
  fun printData(data: ByteArray, width: Long, callback: (Result<Boolean>) -> Unit)
      ...
      run {
        ...
        if (api != null) {
          channel.setMessageHandler { message, reply ->
            ...
            val widthArg = args[1] as Long
            api.printData(dataArg, widthArg) { result: Result<Boolean> ->
              val error = result.exceptionOrNull()
              if (error != null) {
                reply.reply(wrapError(error))
              } else {
